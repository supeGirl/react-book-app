...
var gFilterBy = { txt: '',  minPageCount: 0 }
_createBooks()
...
function query() {
    return storageService.query(BOOK_KEY)
        .then(books => {
            ...
            if (gFilterBy.minPageCount) {
                books = books.filter(book => book.pageCount >= gFilterBy.minPageCount)
            }
            return books
        })
}
...
function setFilterBy(filterBy = {}) {
    ...
    if (filterBy.minPageCount !== undefined) gFilterBy.minPageCount = filterBy.minPageCount
    return gFilterBy
}
